...
const FriendSugCard = ({ friend }) => {
  ...
  const sent_friend_req = async() => {
    ...
    console.log(body)
    try {
        const response = await axios.post("http://localhost:3500/friends/sent_friend_requests", body);
        if (
          response.data.isrequested
        ) {
            setAdd('Request sent')
